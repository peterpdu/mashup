...
def hart_enrichment(x, annot, bin_size=1000, num_bins=100):
    ...
    enrichment = []
    for split in np.split(e, num_bins):
...
def wainberg_enrichment(x, annot, n=10):
    ...
    total_interaction_fraction = (coocc.values > 0).sum() / (coocc.values > 0).any(axis=1) ** 2
    ...
    for i in range(n):
        tp = 0
        # get top n partners
        for top_n in zip(*[x.columns[top_idx[_]] for _ in range(i+1)]):
            # count networks all in same pathway
            tp += (annot[list(top_n)] > 0).all(axis=1).any()
